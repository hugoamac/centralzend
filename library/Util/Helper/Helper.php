...
class Util_Helper_Helper {
            ...
            foreach ($array as $key => $values) {
                switch (true) {
                    case is_array($values):
                        $sAnterior = $key;
                        $array[$key] = Util_Helper_Helper::filtroHTTP($values, $execoes, $sAnterior);
                        $sAnterior = "";
                        break;
                    case is_string($values):
                        if (in_array("$key", $execoes) or in_array("$sAnterior", $execoes)) {
                            if (!get_magic_quotes_gpc()) {
                                $array[$key] = addslashes(mysql_escape_string($values));
                            }
                        } else {
                            if (!get_magic_quotes_gpc())
                                $array[$key] = addslashes(strip_tags(mysql_escape_string($values)));
                            else
                                $array[$key] = strip_tags(mysql_escape_string($values));
                        }

                        break;
                    case is_numeric($values):
                        $array[$key] = (int) $values;
                        break;
                    default:
                        $array[$key] = $values;
                        break;
        ...
        if (empty($array))
            ...
            foreach ($array as $key => $values) {
                if ($values OR ((STRING) $values == "0")) {
                    if (is_array($values)) {
                        $vvNovoArray[$key] = Util_Helper_Helper::limpaArray($values);
                    } else {
    ...
    static public function converteDataParaBanco($sDataHora) {
        if (!empty($sDataHora)) {
            ...
            $sDataHora[0] = Util_Helper_Helper::converteDmaParaAmd($sDataHora[0], "-");
    ...
    static public function converteDataParaPagina($sDataHora) {
        if (!empty($sDataHora)) {
            $sDataHora = explode(" ", trim($sDataHora));
            $sDataHora[0] = Util_Helper_Helper::converteAmdParaDma($sDataHora[0], "/");
            return trim(implode(" ", $sDataHora));
        } else {
            return FALSE;
        }
    ...
    static function nome_para_url_encurtada($sString) {
        $sString = strtolower(ereg_replace("[[:blank:][:space:]]", "-", ereg_replace("(  )", " ", trim(ereg_replace("[[:punct:]]", "", trim(Util_Helper_Helper::removeAcentuacao($sString)))))));
        $characteres = array(
            '¹' => '1', '²' => '2', '³' => '3'
        );
